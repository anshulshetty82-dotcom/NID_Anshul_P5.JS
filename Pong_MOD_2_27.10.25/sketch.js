...
// --- PADDLE VARIABLES ---
let paddleLeftX = 20;
let paddleLeftY = 200; // Initial value matching half of 400px canvas height
let paddleRightX = 580; // For a 600px wide canvas
let paddleRightY = 200; // Initial value matching half of 400px canvas height
let paddleSpeed = 4.8; // Increased from 4 to 4.8 (20% increase)
...
function addBall() {
    ...
    balls.push({
        ...
        y: height / 2,
        speedX: random([-initialSpeed, initialSpeed]),
        speedY: random([-2, 2]),
        size: 10
    });
}
...
function draw() {
    ...
    if (gameState === 'RUNNING') {
        ...
        // 2. BALL SPAWNING LOGIC (Second ball at 30 seconds)
        if (balls.length === 1 && elapsedTime >= THIRD_BALL_TIME) {
            addBall();
        }
        
        // 3. PADDLE MOVEMENT INPUT
        ...
        // 4. ALTRUISM LOGIC (INSTANTANEOUS VISIBILITY)
        ...
        for (let i = balls.length - 1; i >= 0; i--) {
            ...
            if (leftVisibleTimer > 0 && ball.x >= leftCollisionLeft && ball.x <= leftCollisionRight && ball.y >= leftCollisionTop && ball.y <= leftCollisionBottom) {
                ...
                playerScore += 1;
                ball.speedY = (ball.y - paddleLeftY) / 20; // Angle based on hit location
            }
